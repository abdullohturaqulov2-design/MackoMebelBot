...
async def process_add_excel(message: Message, state: FSMContext, bot):
    ...
    preview_lines = ""
    for p in products[:5]:
        preview_lines += f"  • {p['name']} ({p.get('code','') or '-'})\n"
    if len(products) > 5:
        preview_lines += f"  ... va yana {len(products)-5} ta\n"
    if errors:
        preview_lines += f"\n⚠️ {len(errors)} ta qator o'tkazib yuborildi"

    await message.answer(
        t(lang,"excel_confirm", count=len(products), preview=preview_lines.strip()),
        parse_mode="HTML",
...
async def excadd_confirm(callback, state: FSMContext):
    ...
    ids = [db.add_product(**p) for p in products]
    ADDED_CACHE[uid] = ids
    PENDING_ADD_CACHE.pop(uid, None)

    await callback.message.answer(
        t(lang,"added_count", count=len(ids)),
        reply_markup=added_view_keyboard(lang))
    role = get_role_level(uid)
    await state.set_state(AdminStates.main_menu)
    await callback.message.answer(t(lang,"main_menu"), reply_markup=get_main_keyboard(lang,role))
    await callback.answer()

# Tahrirlash — faylni qaytarish
...
async def excadd_edit(callback, state: FSMContext):
    ...
    if path and os.path.exists(path):
        await callback.message.answer_document(
            FSInputFile(path),
            caption=t(lang,"excel_edit_hint"))
...
async def excadd_new_file(message: Message, state: FSMContext, bot):
    if not is_admin(message.from_user.id): return
    # Yangi faylni process qilish — process_add_excel ga yo'naltirish
    await state.set_state(AdminStates.waiting_excel_add)
